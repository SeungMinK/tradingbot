...
HARD_LIMITS = {
    ...
    # #222→#224: ROI 테이블 시간 구간 확장 (30/60/120 → 120/240/600분).
    # 실측 보유 시간 247~781분인데 120분 ROI에 걸려 조기 매도. 봇 매매 주기와 구간 재정렬.
    "roi_10min": (1.5, 6.0),
    "roi_120min": (1.0, 4.0),
    "roi_240min": (0.8, 3.0),
    "roi_600min": (0.5, 2.0),
...
| roi_10min | 1.5 ~ 6.0 | 10분 보유 시 목표 수익률 (%) — 급등 즉시 익절 |
| roi_120min | 1.0 ~ 4.0 | 2시간 보유 시 목표 수익률 (%) |
| roi_240min | 0.8 ~ 3.0 | 4시간 보유 시 목표 수익률 (%) |
| roi_600min | 0.5 ~ 2.0 | 10시간 보유 시 목표 수익률 (%) — **최소 탈출선, 본전치기 금지** |
...
1. **ROI 테이블** — 이길 때 크게 이기는 핵심
   - roi_240min: 4시간 보유 목표 — 이 값이 높을수록 더 오래 보유 → 건당 수익 증가
   - roi_600min: 10시간 최소 탈출선 — 너무 낮으면 본전치기 매도 발생
   - 손절(-5%)이면 roi_240min은 최소 +1.5% 이상 (1:3 비율)
   - **#222~#224: 실측 보유 시간 247~781분인데 기존 120분 ROI에 걸려 조기 매도 빈발.
     시간 구간을 120/240/600분으로 재정렬. 예: roi_240min=2.0, roi_600min=1.5로
     맞춰 건당 평균 수익을 1.13% → 2% 이상으로 끌어올리는 방향.**
...
class LLMAnalyzer:
    ...
    def _apply_recommendations(self, result: dict) -> None:
        ...
        # #224: 시간 구간 확장 10/120/240/600분
        roi_keys = {"roi_10min": 10, "roi_120min": 120, "roi_240min": 240, "roi_600min": 600}
        roi_changed = any(key in params for key in roi_keys)
        if roi_changed:
            roi_table = {10: 3.5, 120: 2.5, 240: 1.8, 600: 1.0}  # #224 시간 확장
            # 기존 DB 값 로드
